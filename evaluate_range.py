    ...
    for i, key in enumerate(keys):
        ...
        for j, model in enumerate(model_names):
            if model != ref_model:
                ...
                emd_score=getEMD(ref_counts,model_counts)
                EMDs[model+"_"+str(key)]=emd_score

                seps = calculate_separation_power(ref_counts, model_counts, bins)
...
#         colors.append(model_to_color_dict[model])
...
def plot_layers(args,model_to_color_dict):
...
if __name__ == "__main__":
    ...
    parser.add_argument('--binning_file', type=str, required=False, 
                        default='/project/biocomplexity/fa7sa/github/Benchmarking_Calorimeter_Shower_Simulation_Generative_AI/xml_binning_files/binning_dataset_2.xml',
    ...
    for i in range(len(target_energies)-1):
        ...
        plot_filename =  'E_layers_dataset_{}_{}.pdf'.format(args.dataset_num, e_range)
        ref = 'Geant4'
        plot_E_group_layers(ref, hlfs, model_names, plot_filename, e_range, args.dataset_num,args)
